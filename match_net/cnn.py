...
class PreTrainedEmbeddding(tf.keras.layers.Layer):
    def __init__(self, embeddings, trainable=False, rate=0.1, **kwargs):
        """"Instantiate the layer using a pre-defined embedding matrix."""
        super().__init__(**kwargs)
        self.embeddings = tf.Variable(embeddings, trainable=trainable)
...
class CnnTextEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, max_features = 5000,
        ...
        if pretrained_embedding is not None:
            self.model.add(PreTrainedEmbeddding(pretrained_embedding, trainable=trainable))
        else:
